  ...
  const handleChapterDragEnd = (result: any, sectionIndex: number) => {
    ...
    const updatedSections = sections.map((section, idx) => {
      if (idx === sectionIndex) {
        ...
        return { ...section, chapters: updatedChapters }; // ✅ 使用新对象更新状态
      }
      return section;
    });

    dispatch(setSections(updatedSections)); // ✅ 更新 Redux 状态
  };
              ...
              <Draggable key={section.sectionId} draggableId={section.sectionId} index={sectionIndex}>
                {(draggableProvider) => (
                  <div
                    ...
                    className={`droppable-section ${sectionIndex % 2 === 0 ? 'droppable-section--even' : 'droppable-section--odd'
                      }`}
                  >
                    <SectionHeader
                      section={section}
